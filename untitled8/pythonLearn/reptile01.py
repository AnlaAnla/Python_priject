...
def formdata(transtr):
    # 待加密信息
    headerstr = '5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.81 Safari/537.36'
    #print(round(time.time()*1000))
    bv=nmd5(headerstr)
    ts=str(round(time.time()*1000))
    salt=ts+'90'
    # 右边那一长串，有道翻译，进入network-》initiator-》fanyi.min.js.formatted->进入了source（或者直接进入。。）-》搜索n.md5
